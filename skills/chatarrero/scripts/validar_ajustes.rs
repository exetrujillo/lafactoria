...
fn main() {
    ...
    if let Some(ajustes) = json_object(&content, "ajustes") {
        if json_number(ajustes, "pausa_entre_pedidos_s").is_none() {
            errores.push("falta la clave numérica 'ajustes.pausa_entre_pedidos_s'".to_string());
        }
        if json_string(ajustes, "navegador_por_defecto").is_none() {
            errores.push("falta la clave de texto 'ajustes.navegador_por_defecto'".to_string());
        }
    }

    // chatarrero es padre de crianza de pulpo-librero (ver "Skills madre" en
    ...
    match json_array(&content, "criados") {
        ...
        Some(criados) => {
            ...
            if entradas.is_empty() {
                errores.push("'criados' está vacío; chatarrero tiene al menos un hijo de crianza".to_string());
            }
